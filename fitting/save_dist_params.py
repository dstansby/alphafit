...
def main():
    ...
    for year in range(1976, 1978):
        ...
        for doy in range(108, 367):
            ...
            print('Loading dists')
            try:
                dists_3D = helios.ion_dists(probe, starttime, endtime,
                                            verbose=True)
                print('Loaded 3D dists')
                dists_1D = helios.integrated_dists(probe, starttime, endtime,
                                                   verbose=True)
                print('Loaded 1D dists')
                distparams = helios.distparams(probe, starttime, endtime,
                                               verbose=True)
            ...
            dists_3D = dists_3D.reorder_levels(['Time', 'E_bin', 'El', 'Az'],
                                               axis=0)
            ...
            dists_3D = dists_3D[dists_3D.index.get_level_values('E_bin') > 3]
            dists_3D = dists_3D[dists_3D.index.get_level_values('E_bin') < 32]
            # Loop through times
            fitlist_1D = []
            fitlist_3D = []
